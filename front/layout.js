function AuthenticatedHeader (render) {
    return render`
        <a
                href="/me"
...
function NotAuthenticatedHeader (render) {
    return render`
        <a
                href="/sign-up"
...
function layout (render, props) {
    ...
    return render`
        <nav class="flex flex-wrap items-center justify-between bg-purple-light px-3 md:px-12 py-4">
            <h1>
        ...
        <aside>${ toast(toastRenderer, props) }</aside>
        <footer class="flex justify-center bg-purple-lighter py-4 px-2">
            <div>
